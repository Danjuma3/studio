...
import { Calculator, Percent, DollarSign, ArrowRight, Lock, Sparkles } from 'lucide-react';
import Link from 'next/link';

export default function CostPercentagePage() {
  const { recipes, ingredients, updateRecipe, subscription } = useInventory();
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-8 animate-in fade-in duration-500">
        ...
        <div className="text-center space-y-4 max-w-lg mx-auto">
          ...
          <p className="text-muted-foreground text-lg">
            Unlock professional food cost percentage analysis and real-time margin tracking to protect your kitchen's profitability.
          </p>
        </div>
        ...
        <Card className="w-full max-w-md border-primary/20 bg-primary/5 shadow-xl">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg bg-white/50 border">
              <CheckCircle2 size={20} className="text-primary" />
              <span className="text-sm font-medium">Profit per Plate Analysis</span>
            </div>
            <div className="flex items-center gap-3 p-3 rounded-lg bg-white/50 border">
              <CheckCircle2 size={20} className="text-primary" />
              <span className="text-sm font-medium">Bulk vs Retail Margin Compare</span>
            </div>
          </CardContent>
          <CardFooter>
...
// Re-importing missing components for the lock screen
